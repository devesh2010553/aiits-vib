...
exports.sendOTPEmail = async (to, otp, name) => {
  ...
  if (!EMAIL_USER || !EMAIL_PASS) {
    throw new Error(
      'EMAIL_USER or EMAIL_PASS not set. Go to Render dashboard → Environment and add:\n' +
      '  EMAIL_USER = yourname@gmail.com\n' +
      '  EMAIL_PASS = your 16-char Gmail App Password (from myaccount.google.com/apppasswords)'
    );
  }
  ...
  const info = await transporter.sendMail({
    from: '"AIITS - Vibrant Academy" <' + EMAIL_USER + '>',
    to: to,
    ...
    html: '<div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto">' +
          ...
          '</div>' +
          '<div style="background:#fff;border:1px solid #e0d0a0;border-top:none;padding:32px;border-radius:0 0 12px 12px">' +
          ...
          '<div style="font-size:40px;font-weight:900;color:#b8860b;letter-spacing:12px;text-align:center;padding:22px 12px;background:#fffbee;border-radius:10px;margin-bottom:20px;border:2px dashed #d4a017">' + otp + '</div>' +
